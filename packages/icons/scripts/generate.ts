...
/**
 * 轻量元数据入口：只含选择器/列表 UI 需要的字段，绝不带 glyphPath。
 * 客户端组件（如编辑器的平台下拉框）从 '@yilink/icons/meta' 引，
 * 避免把 ~130KB 的 SVG 路径全量打进浏览器 bundle。
 */
function metaSource(platforms: readonly GeneratedPlatform[]): string {
...
async function main() {
  ...
  const generatedMetaPath = join(packageDirectory, 'src/generated/platform-meta.ts');
  const nextMetaSource = metaSource(registry);
  const currentMetaSource = await readFile(generatedMetaPath, 'utf8').catch(() => undefined);
  if (currentMetaSource !== nextMetaSource) {
    await writeFile(generatedMetaPath, nextMetaSource, 'utf8');
  }
