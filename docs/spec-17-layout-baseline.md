...
本 spec 体量大，**必须分段落盘、边做边验，禁止把所有改动攒到最后一次性输出**：

1. 每完成一个编号小节（§1 引擎 → §2 数据列 → §3.1 id 稳定化 → §3.2 保存原子性 → §4 基线）立刻写文件到磁盘并跑该节的测试，**通过后再进入下一节**。
2. 每节完成后，向 `/tmp/codex-p0-progress.md` **追加一行**：`[节号] 完成，文件：…，测试：N passed`。这是进度信标，不是最终报告。
3. 最终报告只做汇总，不要在结尾重新生成大段代码。
4. 若某节卡住超过合理时间，跳过并在进度文件记录原因，继续后续节——**部分交付优于零交付**。
