...
export async function rollupDaily(client, now = new Date()) {
  ...
  const retentionStart = addUtcDays(today, -RAW_EVENT_RETENTION_DAYS);
  ...
  // TODO(spec-14): Page.deletedAt 与其 30 天硬清理函数落地后，在这里调用该函数。
...
async function main() {
  ...
  try {
    ...
    console.log(
      `Rolled up ${result.rolledUpPages} page(s); pruned ${result.deletedEvents} raw event(s).`,
    );
  } finally {
...
if (typeof process !== 'undefined' && process.argv[1]?.endsWith('rollup-daily.mjs')) {
  main().catch((error) => {
    console.error(error);
    process.exitCode = 1;
  });
