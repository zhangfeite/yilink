...
async function createTrackableBlock(pageId: string, id = 'event-block') {
  return db.block.create({
    data: {
      ...
      isVisible: true,
      position: 0,
      config: { title: '统计链接', url: 'https://example.com' },
    },
  });
}
...
describe('POST /api/e', () => {
  ...
  it('records a valid event with only coarse, derived request data', async () => {
    ...
    const block = await createTrackableBlock(page.id);

    const response = await POST(
      eventRequest(
        { pageId: page.id, blockId: block.id, kind: 'CLICK' },
  ...
  it('silently drops invalid bodies, bots, and pages that are not published', async () => {
    const user = await db.user.create({
      data: { id: 'event-draft-user', email: 'draft@example.com' },
    });
    ...
    expect(
      [invalid, draftResponse, missingResponse, botResponse].map((response) => response.status),
    ).toEqual([204, 204, 204, 204]);
    await expect(db.clickEvent.count()).resolves.toBe(0);
  });

  it('silently drops a block id that belongs to another page', async () => {
    const user = await db.user.create({
      data: { id: 'event-block-user', email: 'block@example.com' },
    });
    const page = await db.page.create({
      data: { userId: user.id, slug: 'event-block-page', title: '目标主页', status: 'PUBLISHED' },
    });
    const otherPage = await db.page.create({
      data: { userId: user.id, slug: 'other-block-page', title: '另一主页', status: 'PUBLISHED' },
    });
    const otherBlock = await createTrackableBlock(otherPage.id, 'other-page-block');

    const response = await POST(
      eventRequest({ pageId: page.id, blockId: otherBlock.id, kind: 'CLICK' }),
    );
  ...
  it('deduplicates views and caps clicks at 30 per ip, page, and UTC hour', async () => {
    const user = await db.user.create({
      data: { id: 'event-limit-user', email: 'limit@example.com' },
    });
    const page = await db.page.create({
      data: { userId: user.id, slug: 'event-limit-page', title: '限频主页', status: 'PUBLISHED' },
    });
    const block = await createTrackableBlock(page.id, 'event-limit-block');

    const viewResponses = [
      await POST(eventRequest({ pageId: page.id, kind: 'VIEW' })),
      await POST(eventRequest({ pageId: page.id, kind: 'VIEW' })),
    ];
    const clickResponses = [];
    for (let index = 0; index < 31; index += 1) {
      clickResponses.push(
        await POST(eventRequest({ pageId: page.id, blockId: block.id, kind: 'CLICK' })),
      );
    }

    expect(viewResponses.concat(clickResponses).every((response) => response.status === 204)).toBe(
      true,
    );
    await expect(db.clickEvent.count({ where: { pageId: page.id, kind: 'VIEW' } })).resolves.toBe(
      1,
    );
    await expect(db.clickEvent.count({ where: { pageId: page.id, kind: 'CLICK' } })).resolves.toBe(
      30,
    );
  });
});
